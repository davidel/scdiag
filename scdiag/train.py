    ...
    train_augmentations = v2.Compose(
        [
            v2.RandomResizedCrop(
                size=(image_size, image_size), scale=(0.85, 1.0), antialias=True
            ),
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override number of labels (auto-detected by " "default)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="TensorBoard log directory (default: " "<dir_of_latest_ckpt>/logs)",
    )
    parser.add_argument(
        "--log_every",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="GCS URI to sync checkpoints to " "(format: gs://BUCKET/PREFIX)",
    )
...
def train_one_epoch(
    ...
    for batch_idx, (images, targets) in enumerate(dataloader):
        ...
        if (batch_idx + 1) % args.log_every == 0 or (batch_idx + 1) == total_batches:
            ...
            w_top1 = (
                (window_correct / window_samples) * 100.0 if window_samples > 0 else 0.0
            )
    ...
    logging.info(
        f"  Train stats -> loss: {avg_loss:.4f} | top1: {top1:.2f}%"
        f" | time: {elapsed:.1f}s"
    )
...
def main():
    ...
    scaler = (
        ...
        if args.amp_dtype == torch.float16 and device.type == "cuda"
        else None
    )
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            v_loss, v_t1 = evaluate_performance(
