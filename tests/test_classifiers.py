...
class TestBuildClassifier:
  ...
  def test_mlp_custom_hidden(self):
    net = build_classifier(
      "mlp", num_labels=3, hidden_size=128, hidden=256, dropout=0.1
    )
  ...
  def test_cls_attention_default(self):
    net = build_classifier(
      "cls_attention", num_labels=7, hidden_size=64, num_heads=4
    )
    x = torch.randn(2, 10, 64)  # (B, N, D)
    out = net(x)
    assert out.shape == (2, 7)
  ...
  def test_classifier_kwargs_logged(self, caplog):
    with caplog.at_level("INFO"):
      build_classifier(
        "mlp", num_labels=3, hidden_size=64, hidden=128, dropout=0.5
      )
