...
class TestCreateScheduler:
  ...
  def test_cosine_warmup(self, optimizer):
    sched = create_scheduler(optimizer, name="cosine_warmup", warmup_epochs=5)
  ...
  def test_cosine_warmup_no_warmup_epochs(self, optimizer):
    ...
    sched = create_scheduler(optimizer, name="cosine_warmup", warmup_epochs=0)
    assert isinstance(sched, optim.lr_scheduler.CosineAnnealingLR)
  ...
  def test_cosine_eta_min_override(self, optimizer):
    sched = create_scheduler(optimizer, name="cosine", epochs=100, base_lr=1e-3)
  ...
  def test_step_scheduler_params(self, optimizer):
    sched = create_scheduler(optimizer, name="step", step_size=5, gamma=0.5)
